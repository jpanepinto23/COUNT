...
function RewardCard({ reward, user, redeeming, onRedeem }: {
  ...
  const logoUrl = `https://icon.horse/icon/${new URL(reward.affiliate_url.startsWith('http') ? reward.affiliate_url : 'https://' + reward.affiliate_url).hostname}`
    ...
    <div style={{ borderRadius: 14, overflow: 'hidden', border: '1.5px solid #E0D9CE', background: '#fff', opacity: canAfford ? 1 : 0.75 }}>
      ...
      <div style={{ padding: '12px 14px', display: 'flex', alignItems: 'center', gap: 10 }}>
        ...
        <div style={{ flex: 1, minWidth: 0 }}>
          ...
          <p style={{ fontSize: 11, color: '#8A8478' }}>{reward.brand_name} · {reward.retail_value > 0 ? `$${reward.retail_value} value` : reward.description}</p>
        </div>
        <button onClick={() => onRedeem(reward)} disabled={!canAfford || redeeming === reward.id} style={{
          ...
          {isRedeeming ? '…' : `${reward.point_cost.toLocaleString()} pts`}
        </button>
      </div>
    </div>
  )
